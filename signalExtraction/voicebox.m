...
%      Version: $Id: voicebox.m,v 1.16 2009/09/13 15:52:07 dmb Exp $
%
...
if isempty(PP)
    ...
    PP.dir_temp='/var/folders/v1/v1DHWMNlHqme4CSZ3GDh2k+++TI/-Tmp-/'; % directory for storing temporary files'
    PP.dir_data='/Users/Tanguero/Documents/MSC/MSc-Dissertation/signalExtraction/AdditionalFiles';           % default directory to preappend to speech data file names
    PP.shorten='/Users/Tanguero/Documents/MSC/MSc-Dissertation/signalExtraction/shorten/shorten.1';            % location of shorten executable
    PP.flac='/Users/Tanguero/Documents/MSC/MSc-Dissertation/signalExtraction/flac-1.2.1b/flac-1.2.1b.exe';                  % location of flac executable
   ...
   if exist(PP.dir_temp)~=7        % check that temp directory exists
    ...
    if exist(PP.shorten)~=2        % check that shorten executable exists
        PP.shorten=fullfile(fnp,'shorten.1'); % next try local directory
        if exist(PP.shorten)~=2        % check if it exists in local directory
            PP.shorten='shorten.1'; % finally assume it is on the search path
        end
    end

    if exist(PP.flac)~=2        % check that flac executable exists
        PP.flac=fullfile(fnp,'flac-1.2.1b.exe'); % next try local directory
        if exist(PP.flac)~=2        % check if it exists in local directory
            PP.shorten='flac-1.2.1b.exe'; % finally assume it is on the search path
        end
    end
