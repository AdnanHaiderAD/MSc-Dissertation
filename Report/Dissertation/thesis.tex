...
\include{chap5}
 ...
 High dimensional data vectors $\{x_n\}$  typically lie close to a non- linear manifold \citep{bishop2006pattern} whose intrinsic dimensionality is smaller than that of the input space as a result of strong correlations between the input variables. In data mining, to understand this intrinsic dimensionality of the data, the Single Value Decomposition(SVD) algorithm is popularly used. SVD gains an understanding of a large database by simplifying the data representation by showing only the number of important dimensions that captures the \textbf{variance} of the data. Intuitively speaking, SVD projects the high dimensional data vectors to an orthogonal subspace that maximises the variance of the data. Recent works\citep{Korn1997} have shown that SVD can be applied to achieve dimensionality reduction in time sequences given that  the sequences satisfy certain constraints. Applying SVD as a preprocessing step  to such time series data sets can result in a significant reduction in the runtime of the algorithm.  The focus of this chapter is to investigate compatible preprocessing methods that can be used alongside SVD to improve the accuracy of the 1 nearest neighbour classifier  and minimise its run time.
  
 
 \section{Motivation}
Unlike other data sets, clustering/classifying time sequences by conventional methods such as the euclidean metric in most cases results in very poor accuracy. The scope of such metrics is quite  limited in the fact that they  require  sequences to share the same length and they can only perform comparison by conducting a linear match of the  temporal dimensions of the two sequences. This represents a  substantial drawback as the metrics fail to take into account that similar patterns  may vary in terms of speed,length and scale. Therefore, this provides the motivation to look for  preprocessing feature extraction techniques that can extract translation and scale invariant features that capture information about  the intrinsic trends in the data.

However, recent works have shown that there exists time series domains where the use of conventional methods can still achieve accurate results. Korn in his paper  \citep{Korn1997} has  shown that  for time series sequences that share the same \textbf{speed} and \textbf{length},  the conventional methods can achieve good clustering/classification performance. If  sequences of the same class share the same speed then they will also share the same time localised trends(figure 6.1). Applying SVD as a preprocessing step,  results in the  extraction of  latent factors that represent the  localised patterns in time.  Through mapping the time series sequences into the latent space, we can therefore construct a  simplified representation of the data which allows conventional methods such as the euclidean metric to cluster time sequences with less run time . 
...
\begin{enumerate}
\item Time complexity: The time complexity of SVD  is O($D^3$) where D represents the dimensionality of the data. For application domains such as  time series data where the number of samples $\ll$ than the length of the sequences i.e the dimensionality of the sequences, the high computational cost  associated with performing SVD increases the run time of the classification algorithm. 
...
\item Dimensionality:  SVD cannot be directly applied to datasets where the individual data points do not share the same dimensionality. This issue is specially seen in time series data sets such as speech. The acoustics samples in such data sets vary in length.
\end{enumerate}

To address the above issues and the drawbacks associated with performing nearest neighbours classification using conventional distance metrics, in the following three sections, I investigate :
\begin{enumerate}[label=\roman{*}]
...
To minimise the time complexity, Bishop\citep{bishop2006pattern} offers the following solution:
 
 If X  is  D by N  data matrix where D is the number dimensions and N is the number of samples and  D$\ll$N, the X will have at least N-1 singular values. In the previous chapter we have seen that when X factorizes to $U \times D \times V^T$ we can  achieve dimensionality reduction by considering only a subset of the columns vectors of V   that are associated with the non-zero singular values. The columns vectors of V are the eigenvectors  of the covariance matrix.Thus, dimensionality reduction can be achieved by mapping the data points to latent space spanned by the eigenvectors corresponding to the non-zero eigenvalues. 

The eigen-decomposition of the covariance matrix is $ XX^T u_i= \lambda_i u_i$. $XX^T$ is N by N matrix and the computational complexity of computing the eigenvectors is O ($D^3$).
...
From the above expression, it can be observed that the eigenvectors of  $X^TX$ and  $XX^T$ share the same eigenvalues. Thus we can solve the eigenvector problem in spaces of lower dimensionality with computational cost O($N^3$) instead of O($D^3$). The required eigenvectors of the covariance matrix can then be computed by the using the following expression:
\[ u_i = \frac{1} {\lambda^(\frac{1}{2})} Xv_i\]
 
To verify the above approach,  I have conducted the following experiment :

\textbf{Objective} : The experiment is designed to verify  whether applying SVD as  a preprocessing step reduces the run-time of the classifier without decreasing the accuracy.

\textbf{Data sets used} : InLineSkate and Cinc\_ECG\_TORSO. 
Unlike the speech corpus, the intra class variance of the two UCR data sets is much lower as sequences of the same class share the same speed shown by the figure below. Figure 6.1 shows examples of two sequences belonging to class `1' in the  Cinc\_ECG\_TORSO data set. Apart from sharing the shape, the  two sequences share the  same speed which in  other words means  that they share the same time localised trends. This allows classification algorithms to treat the problem as a  normal classification problem. Since sequences with in each data set share the same length, SVD can directly be applied to achieve dimensionality reduction. For this experiment, SVD has been applied to extract all the eigenvectors of the covariance matrix that corresponds to non-zero singular values. Since the number of sample $\ll$ the length of the sequences, the size of the latent space will be smaller than the original input space leading to dimensionality reduction without any loss of information. 
\begin{figure}[H]
  \centering
...
\textbf{Observation}
From the  experimental results, applying SVD using bishops approach as a preprocessing step reduces the run time of the nearest neighbour algorithm. 


 as the sequences within data set share the same length.
