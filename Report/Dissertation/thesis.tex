...
\end{enumerate}
An inspection of the kernel function shows it represents the sum of dot products of vectors. An  ideal cost function that make use of dot products is the \emph{arc-cosine}. Using an appropriate substitution, I have thus constructed the following cost metric:  
\[ \theta = \frac{ <X,Z>}{|X||Z|} \]
...
The goal here is to improve the performance of the DTW algorithm in problem domains where minimising time complexity has a high priority.
To investigate whether the proposed changes provide a better alternative to using the Sakoe-Chuba band window constraint, I have conducted the following experiments:
