...
\textbf{Observation}
\begin{itemize}
\item  Allowing the 1 NN classifier to exploit information about the shape of the envelopes improves the ability of  the Euclidean metric to idetify intra class speech utterances more accurately by performing just a linear match between the  sequences. From figure 8.4, we can observe embedding the multi step feature extraction methodology improves the accuracy of the classifier by 6\%.
...
\item  The  1 NN classifier  achieves almost near perfect accuracy when it uses the domain dependent MFCC features.  This shows that when employing the DTW metric to match speech utterances, the MFCC feature vectors are the ideal features to use to distinguish utterances of different lexical identities.
 ...
 intra class speech utterances, the MFCC feature vectors are in fact the ideal feature vectors to use if we are aiming to minimise the classification error rate.   
