...
The formal definition of the curvature of a curve is as follows:
...
To extract the curvature at each point of the time series sequence,I performed a two step feature extraction process.

\textbf{Step 1}:

 In the first step, I  have fitted a \textbf{smoothed cubic spline} to each sequence. A spline is a sufficiently smooth polynomial function that is piecewise-defined, and possesses a high degree of smoothness at the places where the polynomial pieces connect []. A cubic spline is a type of spline which is  constructed using piecewise  polynomials of order 3. The objective here, is to extract features that represent the \textbf{global shape} of the sequence, thus for this problem, the shape information of local trends is not relevant. 

To ensure that curvature values represent  information more about the global shape rather than the local trends, I have used smoothed cublic spline to fit the data.
Mathematically, the smoothing spline estimate $\hat g(x)$ of the actual function $g(x)$ is defined to be the minimizer of :
\[
\lambda \sum_{i=1}^n \{ y_i - g(x_i) \}^2 + (1-\lambda) \int_a^b \{g''(t)\}^2 \,
dt
\]
...
The critical parameter here is $\lambda$. The values of $\lambda$ determines the nature of the fit. For the sequences in the  InLineSkate dataset, exact fitting is unnecessary as we are interested only the shape of the dominant global trends. Thus, in  this situation an ideal value of $\lambda$ will be than half.  For my analysis, I have chosen the value of 0.25.
 \end{spacing} 
%% ... etc ...
