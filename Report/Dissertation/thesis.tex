...
Unsupervised Pattern discovery in high-dimensional time series data.
The term 'Time Series' means a set of observations concurring any activity against different  periods of time. The duration of time period may be in the order of microseconds to monthly or annually depending of the domain.
Mathematically, a time series is defined by the values $Y_1, Y_2...$ at times $t_1,t-2...$  where $Y=f(t)$
The goal of motif Þnding is the detection of novel, unknown signals in a set of sequences. For example
you might want to detect transcription factor binding sites in a genome, but do not know what they
look like or in the case of speech, detect the mannerism, the most frequent words spoken by the speaker.


In time series, time act as an independent variable to estimate dependent variables

The mining of time-series data has received considerable attention in the last decade. One particular aspect of time series analysis that is currently very popular is \emph{motif} discovery- the problem of efficiently locating frequent/interesting sub-patterns in the data.  The knowledge of motifs has been seen  to have important applications in various aspects of data mining tasks. For instance:
\begin{itemize}
