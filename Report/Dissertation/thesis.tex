...
\title{Improving The Performance Of  Similarity Metrics  In Time Series Domains}
...
\abstract{%
 ...
 The mining of time-series data has received considerable attention within the  data mining and machine learning community in the last decade. Time series analysis is used in many applications ranging from  sales forecasting, budgetary analysis to stock market prediction and many more. To identify and compare frequently occurring patterns,  a popular metric that is frequently used is the Dynamic Time Warping(DTW) algorithm. Unlike normal datasets, time series sequences are very hard to compare. Intra class sequences  vary in terms of size, scale and speed thus making the use of  standard metrics such as the Euclidean distance  very limited in scope.  The DTW algorithm on the other hand is not restricted to performing just a linear match of the temporal dimensions of sequences. By warping the time axis, the algorithm compares events localised in different regions of time. However, one of the fundamental drawbacks of using this algorithm is its high time complexity. The time complexity of the DTW is O($n^2$) where n is the length of  the sequences. For long time series sequences, the run time incurred in using the DTW algorithm is  thus quite high which makes  the algorithm  an unattractive choice in comparing high dimensional time series sequences. In such scenarios, the Euclidean distance however is a more attractive choice since it has a time complexity of O(n).  To address the drawbacks associated with both metrics, in this project, we present procedures to improve both  the accuracy and speed of both metrics  in handling high dimensional time series sequences. We present an adaptation of the DTW that is faster and yet more accurate than the standard DTW algorithms using the most rigid window constraints. We also present a feature extraction procedure that allows the Euclidean metric to be as accurate as the DTW algorithm applied on raw sequences.
...
First and foremost I will like to thank my supervisor Dr. Sharon Goldwater for her continuous support through out this project. I will like to dedicate this project to two people: my father and the public speaker  Ray lewis.  To do good research, an individual needs be very diligent and patient with his work. This is not an easy thing to do. At times,  the work can be tedious and highly frustrating but one should never give up. Skill is something that comes after hours and hours of working on one's craft. Mr. Ray lewis, I will like to personally thank you for inspiring me through your public speeches to always give my 100 \% no matter what the outcome. And Dad, I have you to thank for teaching me to be calm  even at the hardest times. The way you calmly assess tough scenarios is  a remarkable trait that I have come to admire.
...
The  dissertation is  organised as follows: Chapter 2 gives a brief description of the main time-series datasets used for this project. Chapter 3  provides a detailed background description of the DTW algorithm.  Chapter 4 and 5 investigates methods to improve the accuracy and speed of the DTW algorithm in handling high dimensional time series sequences. From chapter 6 onwards, the focus is primarily on improving the performance of the Euclidean metric in correctly identifying intra class time series sequences. Chapter 6 explores  how single value  decomposition can be used to minimise  the run time of the 1NN classification  using the Euclidean metric. In chapter 7,  we explore feature extraction techniques to improve the accuracy in using the Euclidean metric to identify similar patterns. In Chapter 8, we extend these techniques  to extract useful features from speech utterances of variable lengths, thus allowing the Euclidean metric to be successfully applied to speech datasets.
 Finally in the last chapter, I present my conclusions and present scope for future work.
...
the Itakura parallelogram \citep{Itakura1975} . Figure 3.2  gives an illustration  of the use of both constraints:
  ...
  \centering
     ...
     \caption{ Two constraints: Sakoe-Chiba Band (left) and an Itakura Parallelogram (right), both have a width of 5.}
    \end{figure}
...
\begin{center} 
 ...
 women & 513 \\
  ...
  The  main objective of my research is to improve the accuracy of the DTW algorithm while reducing the time and computational cost to a \textbf{ minimum}. Even after applying the feature selection process, from initial experiments on few samples, I have found that the computational cost incurred by the algorithm is still very high. Hence to minimise the run time, I  employed the Sakoe-Chiba band that has an adaptive   window size of  : w = max($\lceil{0.1*max(n.m)}\rceil$, abs(n-m)). 
...
\item the use of the rigid window constraint- the  low accuracy may be attributed to the optimum warping path between similar patterns lying outside boundaries of the Sakoe-Chiba bands\citep{Zhang2010,Park2008}.
...
\begin{figure}[H]
 ...
 -The run-time on the other hand has improved considerably. The average ln(run time) has decreased by 5.7. This is obvious since the acoustic signal is mapped to smaller sequence of frames.  
  ...
  \item Removing redundant features have greater relative impact on the accuracy than employing domain dependent MFCC features. This is evident  if we compare the accuracy of the 1NN classifier  when it uses the  method \textbf{M2} against the accuracy achieved when it uses  method \textbf{M3}. The presence of silence regions force the optimal warping paths between  patterns of the same lexical identity to lie outside the regions in the cost matrix bounded by the Sakoe-Chiba band. This forces the DTW to find an \textbf{approximate} optimal path with in the allowed regions which in turn leads to an increase in the error rate.
...
Since the main goal is to improve  \textbf{both} the accuracy and  speed of DTW in handling sequences of high-dimensionality i.e long lengths, removal of silence segments provides an ideal mechanism to improve  \textbf{both}  the time complexity and the accuracy of the algorithm. Dropping the window constraint on the other hand, will although lead to considerable increase in the accuracy by will result longer run-times. This is not ideal when working on large high dimensional datasets.
  ...
  \item Employing Method \textbf{M5} achieves almost near perfect accuracy. Dropping the window constraint of the DTW improves the accuracy of the 1NN classifier by 67.15\% from the accuracy achieved by the classifier when it employs method \textbf{M4}.  In other words 67.15\% of the  time on average, the optimum warping path lie outside the regions bounded by the Sakoe-Chiba band constraints, This confirms  that  patterns belonging to the same lexical identity can have an \emph{overly temporal askew} between them as result of the different contexts in which the words are spoken and/or as a result of different speakers speaking the same word.
...
To investigate whether the proposed changes does provide a better alternative to using the Sakoe-Chiba band window constraint, I have conducted the following experiments:
...
\textbf{Variables being compared}: Sakoe-Chiba band window constraint VS the proposed methodology
...
\begin{itemize}
\item In comparison to employing the Sakoe-Chiba band window constraint, the alternative method allows DTW to improve its accuracy across all 4 categories by 6.54\% on average.
...
\item Decreasing the size of the time slices only leads to a minimal increase in accuracy. Thus using the default value of 50  seems to be safe option as the algorithm better accuracy and speed than using the rigid Sakoe-Chiba band constraint .
...
This proves  that by applying the  wavelet transform as a preprocessing step, the 1NN classifier equipped with the Euclidean metric can identify intra class sequences that  roughly share the same time aligned global trends with  high accuracy without being subjected to high run times.
...
\end{itemize}
 ...
 \caption{The green plot corresponds to signal constructed from $A_1$ and the blue plot presents the original signal }
  \end{figure}
 The accuracy of the model was compared against the accuracy of the baseline 1NN classifier using the Euclidean metric and the  accuracy of 1NN classifier using the DTW algorithm\citep{Xie2010}.
...
\begin{figure}[H]
 ...
 \caption{The top plot corresponds to an utterance of digit 5. The second plot corresponds to the envelope of the signal extracted by the Hilbert transform. The third plot corresponds to a smoothed version of the envelope extracted by wavelet decomposition   }
  ...
  By combining the envelope extraction method with the feature extraction methods discussed in section 8.2, we now have a methodology to extract  features that capture the shape of the envelopes of a signal. To test this idea, I have conducted the following experiment:
 ...
 For each utterance:  
  \begin{itemize}
  \item Remove the regions that correspond to silence. In section 4.1, through exploratory data analysis we have identified that the silence regions correspond to redundant features that play no part in distinguishing intra class sequences. Thus removing such parts from the signal achieves dimensionality reduction without any loss of information. We can  refer to this phase of the feature extraction process as the data cleaning step.
 \item Apply the matlab function `resample' to rescale the utterance to the mean length of the sequences in the training dataset. This ensures that  all sequences to share the same length while at the same time preserving the shape of the envelopes associated with each utterance.  
  ...
  %The extracted sequences however, still suffer from very high dimensionality.  Since the  objective here is to extract the global shape of the envelope, by  following the methodology discussed in 7.2.2, I have taken the first k fourier coefficients from the fourier transform  of the signal.  As I have already mentioned in the section 7.2, the  information of the global shape is captured by the fourier coefficients of the lower frequencies\citep{Osowski2002}. For this dataset, by conducting multple experiments, I have found that taking the first 77 fourier coefficients yield the highest results.
   \end{itemize}
\textbf{Run time of preprocessing}  : The time taken to apply this above feature extraction methods is 63.5s. 


To investigate whether the proposed feature extraction methodology improves the ability of the Euclidean metric to identify intra class sequences, I have compared  the accuracy and the run time of the classifier against the performance achieved by  the  baseline 1NN classifier combined with SVD employed on the rescaled speech utterances. A summary of the results are as follows:
 \begin{table} [H]
 \centering
\begin{tabular}{|c|c|c|c|}
...
\textbf{Observation}
\begin{itemize}
\item  Allowing the 1 NN classifier to exploit information about the shape of the envelopes improves the ability of  the Euclidean metric to identify intra class speech utterances more accurately by performing just a linear match between the  sequences. From figure 8.4, we can observe embedding the multi step feature extraction methodology improves the accuracy of the classifier by 6\%.

\item Applying the multistep feature extraction  as a preprocessing step, reduces the run time of the 1NN classifier. However, the preprocessing step on its own is a bit time consuming as we applying multi level wavelet decomposition following by curvature fitting on each utterance at a time.

  \end{itemize}
As a  final experiment, I replaced the Euclidean metric and re-ran the whole experiment  again using the DTW metric. The results were compared with the  accuracy and run time of the DTW algorithm using the standard MFCC features. This allows us to analyse  how well does the proposed feature extraction methodology  do in capturing common attributes from the intra class utterances in comparison to the MFCC vectors.  A summary of the results are as follows:
\begin{table}[H]
\begin{tabular}{|c|c|c|c|}

\hline \\
Proposed feature extraction  + DTW & MFFC features + DTW\\\\
\hline\\
   3023s & 171283s\\
\hline
\end{tabular}
...
\begin{itemize}
\item Judging from the run times, we can conclude that the proposed multi step feature extraction process achieves greater dimensionality reduction than the MFCC feature vectors. 

\item  The  1 NN classifier  achieves almost near perfect accuracy when it uses the domain dependent MFCC features.  This shows that when employing the DTW metric to match speech utterances, the MFCC feature vectors are the ideal features to use to distinguish utterances of different lexical identities.



  
\item By comparing the experimental results with the results of the previous experiment, it can be  observed  that the multi-step feature extraction method  favours the Euclidean metric over the DTW.  Under the proposed extraction method, the accuracy of the 1NN classifier using the DTW metric is almost 9\% less than the accuracy achieved by the classifier when it uses the Euclidean distance.
...
\end{itemize}
 And finally  in chapter 9, we investigate how this multistep feature extraction process can be extended to exploit information about the shape of envelopes to help the Euclidean metric to identify speech utterances corresponding to the same lexical identity with greater accuracy. The constructed method also allows the Euclidean metric to be successfully applied to time series datasets where  sequences  have different lengths. One of the main drawbacks associated with this approach is its inability to scale to speech datasets with larger vocabularies. The approach assumes that  the utterances associated with each lexical identity share a unique envelope but this assumption doesn't necessarily hold for large vocabulary speech datasets. In a large vocabulary setting,   words even if they are not homophones may possess  similar envelopes as a result of sharing  similar patterns of vowels and consonants. This reduces the ability  of the Euclidean metric to identify intra class sequences with good accuracy. 
...
\item I have successfully constructed a multi-step feature extraction scheme  that extracts  global shape descriptors of time series  sequences. Experimental results have shown that this proposed feature extraction method allows the  Euclidean metric to be as effective as the DTW metric applied on raw sequences in time series problem domains where intra class sequences  are distinguished by their global shape.
\end{itemize}
      \end{spacing}
