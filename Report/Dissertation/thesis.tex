...
\item 1NN classifier equipped with euclidean metric achieves very high accuracy for the Cinc\_TORSO\_ECG dataset. From the accuracy score, we can conclude that intra class sequences    tend to  share the similar \textbf{time localised } local  and global trends since sequences to be classified fairly accurately by performing only a linear match of their temporal dimensions.
\item Using the euclidean metric, the 1 NN classifier is seen to achieve very low accuracy for InlineSkate dataset.This shows that unlike the Cinc\_TORSO\_ECG datasets, the intra class sequences do not share \textbf{both} local and global trends.
