...
\begin{cases}
 ...
 Here D is a diagonal matrix of singular values ranked from greatest to least. These determine the `rank' of the original matrix.
...
Unlike other data sets, clustering time sequences by conventional methods such as the euclidean metric achieves very poor results. The euclidean metric or any of the other conventional methods compare sequences based on a linear match of their two temporal dimensions. This is a huge drawback the metrics fail to take into account similar patterns even if they are of the same length may vary in terms of speed and scale. Hence the use of conventional metrics  is  pointless when it comes to  clustering time-series  patterns. To address this issue, Korn in his paper  \citep{Korn1997} have shown that SVD can be  also applied to efficiently perform time series indexing. Applying SVD as a preprocessing step, can result in the  extraction of  latent factors that represent patterns in time. Through mapping the time series sequences in the space  spanned by these latent factors, we can therefore construct a representation of the data which allows conventional methods such as the euclidean metric to cluster time sequences with greater accuracy.

To motivate this reasoning, lets consider the  following toy example :
...
\textbf{Observation}
\begin{itemize}
\item From an observation of the matrix, it can seen that there are two groups of customers: one group  makes calls only during week days while the other group only makes calls during weekends
\end{itemize}
