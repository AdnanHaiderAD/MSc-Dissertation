...
\begin{figure}[H]
  \centering
     ...
     \includegraphics[scale=0.6]{425.jpg}
  \caption{}
...
The changes that I have introduced  in the previous section  to the `Dynamic Time Warping' algorithm is aimed to improve the algorithm's performance in handling long time series sequences. In this section, I investigate  the performance of this proposed  methodology against  the versions of the DTW algorithm that employ the Sakoe-Chuba band constraint (discussed in 4.1.1). The window constraint that I have used so far is the most rigid constraint that can be imposed on the DTW algorithm without forcing the algorithm to behave like the euclidean metric. In order to compare  the performance of using my proposed method  with that of using  the window constraint, I have conducted the following 3 experiments:
\textbf{Experiment 1}

In the first experiment I , I  take the model discussed in 5.1  for the TIDIGITS data set which consists of applying two stage feature selection process which involves  silence removal and downsampling followed by the feature extraction phase of [2] and investigate the differences in the performance  between the DTW algorithm is equipped with the changes I propose and  the DTW algorithm using a window constraint.
The results found from the experiments are as follows:
