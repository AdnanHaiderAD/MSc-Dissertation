...
The data matrix X can be factorized as $X = U\times D'\times V'^T$ where D is now an M by K diagonal matrix containing only positive values on its diagonal and V' is the corresponding N by K matrix. The matrix $V'^TV$ captures the full  covariance of the data. 
 ...
 \section{Motivation}
Unlike other data sets, clustering time sequences by conventional methods such as the euclidean metric achieves very poor results. Such metrics compare sequences based on a linear match of their  temporal dimensions assuming that they share the same length. This represents a  substantial drawback as  the metrics fail to take into account that similar patterns  may vary in terms of speed and scale. Hence the use of conventional metrics  is  pointless when it comes to  clustering time-series  patterns. To address this issue, Korn in his paper  \citep{Korn1997} have shown that SVD can be  applied as a preprocessing step to successfully perform time series indexing. Applying SVD as a preprocessing step,  results in the  extraction of  latent factors that represent patterns in time. Through mapping the time series sequences into the latent space, we can therefore construct a  simplified representation of the data which allows conventional methods such as the euclidean metric to cluster time sequences with greater accuracy.

Lets motivate this approach by considering the following example :
...
\textbf{Observation}
\begin{itemize}
\item From  observation of the matrix, it can seen that there are two groups of customers: one group  makes calls only during week days while the other group makes calls only  during weekends
\end{itemize}
...
\end{itemize}
Thus from the toy example it can be seen that SVD can be applied to extract patterns in time as well as samples  from time series sequences.


\section {Baseline Implementation}
To apply SVD to time series data, there are two fundamental issues that must be addressed :
\begin{enumerate}
\item The first issue is  time complexity. The time complexity of SVD  is O($D^3$) where D represents the dimensionality of the data. For data sets such time series data where the number of samples $\ll$ than the length of the sequences i.e the dimensionality of the sequences, the high computational cost  associated with the SVD makes it a very unattractive algorithm to use. 
\item The second issue is that SVD cannot be directly applied to datasets where the individual data points do not share the same dimensionality. This issue is specially seen in time series data sets such as speech. The acoustics samples in such data sets vary in length.
\end{enumerate}
For now, I will concentrate on addressing the first issue:
