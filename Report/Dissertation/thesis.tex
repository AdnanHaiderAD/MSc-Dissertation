...
\begin{enumerate}
\item Dynamic time warping  algorithm(DTW) [2,3,4,5,6,11] that clusters similar sequences separated by time shifts or scale.
\item Single value decomposition(SVD)[12]. The entire time series data set can be approximated by a low-rank approximation matrix achieved through transforming and mapping the data onto a lower dimensional orthogonal feature space.
\end{enumerate}
