...
However, recent works have shown that there exists time series domains where the use of conventional methods can still achieve accurate results. Korn in his paper  \citep{Korn1997} has  shown that  for time series sequences that share the same \textbf{speed} and \textbf{length},  the conventional methods can achieve good clustering/classification performance. If  sequences of the same class share the same speed then they will also share the same time localised trends. Applying SVD as a preprocessing step,  results in the  extraction of  latent factors that represent the  localised patterns in time.  Through mapping the time series sequences into the latent space, we can therefore construct a  simplified representation of the data which allows conventional methods such as the euclidean metric to cluster time sequences with less run time . 
...
\begin{enumerate}
\item Time complexity: The time complexity of SVD  is O($D^3$) where D represents the dimensionality of the data. For data sets such time series data where the number of samples $\ll$ than the length of the sequences i.e the dimensionality of the sequences, the high computational cost  associated with the SVD makes it a very unattractive choice for a feature extraction method. 
...
\textbf{Objective} : Measure the effect  of using SVD on the  accuracy and run-time of the 1 nearest neighbour classifier.

\textbf{Data sets used} : InLineSkate and Cinc\_ECG\_TORSO. Unlike the speech corpus, the  time series sequences within  each  of these data sets share the same length.

\textbf{Models compared} : baseline 1 Nearest neighbour classifier VS  baseline 1 nearest classifier employing SVD to achieve dimensionality reduction as a preporcessing step: 

\textbf{Results}:
\begin{tabular}{|c|c|c|}
\hline \\
Accuracy  & raw features & latent features \\
\hline\\
