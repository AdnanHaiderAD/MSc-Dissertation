...
In the first 5 chapters, the focus has been primarily in improving  the  speed and accuracy of the DTW algorithm  in handling sequences that have very long lengths. In Chapter 4, we explored   domain dependent feature extraction techniques  that can  improve the performance of the baseline DTW algorithm. From the results gathered from numerous experiments, we  have observed  the DTW favours domain dependent  multivariate  feature sequences, such as the MFCC feature vectors in the case  of the TIDIGITS dataset, in  constructing optimal paths for intra class sequences which are much closer  to the diagonal regions of the cost matrix. In Chapter 5, the focus has been to improve the DTW algorithm itself. Section 7.2 proposes an alternative adaptation to the DTW algorithm  than augmenting it with window constraints  when tackling problem domains where the time complexity is of high priority. From conducting  various experiments, we have seen that the proposed adaption yields higher performance in both accuracy and run time when combined with both univariate and multivariate  features sequences. Due to the lack of time, we didn't manage to test the proposed DTW implementation on detecting motifs from time series sequences. For future work, it will be thus  interesting to test the proposed implementation in a unsupervised context and  investigate how the proposed adapted version of the DTW algorithm compares to the  baseline version augmented with window constraints in identifying motifs from long time series sequences. 


 
  The new methodology makes use a self proposed kernel that allows the algorithm to yield high irrespective if it is using multivariate or uni variate features. 

From experimental results, we have seen that the proposed adaption allows DTW to achieve both higher accur

through conducting several experiments, we have  observed that using domain dependent preprocessing techniques  prior to applying DTW can  improve its performance in terms of both accuracy and speed with out the need of any global constraints. The DTW algorithm can be applied to both univariate and multivariate feature sequences. However experimental results have shown that algorithm yields higher accuracies when it is applied to multivariate feature sequences. For the TIDIGITS dataset, in section 4.2, we have observed that replacing the raw values with the MFCC feature vectors improves the classification accuracy even 	window constraints are employed to reduce the search space. 


In the second half of the project, we have observed how a mutlistep feature extraction procedure can help improve both the accuracy and the run time of 1NN classifier using the Euclidean metric. The multi step feature extraction procedure  discussed in section 7.2  has been designed to improve the  in problem domains where intra class sequences are distinguished by their global shapes.

One of the main drawbacks of the proposed  multi step feature extraction methodology discussed in chapter 7 and 8 is the fourier  feature extraction phase. The fourier extraction phase is applied to extract the information about the global shape of the sequences. in scenarios where the resultant feature sequences extracted by performing wavelet decomposition followed by  the fourier extraction is applied as dimensionality reduction step to extract information about the global shape of the sequence from the sequence of extracted curvature values.  The UCT dataset InlineSkate is an example of a problem domain where the use of the feature extraction phase cannot be avoided.While the decomposition level of applying the wavelet transform can be decided by conducting exploratory data analysis, to choose the appropriate number of fourier coefficients, we require a labelled training set. 



For problem domains where the use of fourier feature  extraction phase can not be avoided, the proposed multi-step feature extraction suffers from a essential drawback. While the decomposition level of applying the wavelet transform can be decided by conducting exploratory data analysis, to choose the appropriate number of fourier coefficients, we require a labelled training set.
