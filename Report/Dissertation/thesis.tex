...
\begin{cases}
 ...
 \end{itemize}
For time series data sets, very little research has been done to apply SVD to time series sequences. Clustering time sequences by conventional methods is extremely difficult. The dimensions in the time series sequences correspond to time indices and as I mentioned earlier, similar patterns even if they are of the same length may vary in terms of speed and scale. This makes the use of euclidean metric in clustering patterns obsolete.  Korn in his paper  \citep{Korn1997} have shown that SVD can be  also applied to efficiently perform time series indexing. Applying SVD as a preprocessing step, can help in extracting patterns in time  that can be used to construct a simplified representation of the data. By presenting each time series sequences in terms of the dominant patterns, we can thus apply the euclidean metric to efficiently cluster sequences. 

Let motivate the considering the  following toy example :
