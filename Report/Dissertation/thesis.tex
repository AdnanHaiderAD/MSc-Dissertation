...
Theoretically, the integral is evaluated as a Cauchy principal value\citep{RamP.Kanwal1996}. The above integral corresponds to  the  convolution of the function $\frac{1}{\pi t}$ with the function f(x) :
\[F(t) = \frac{1}{(\pi*t)} * f(t)\]
which by the convolution theorem of Fourier transforms, can be also evaluated  as the product of the transform of f(x) with $-i*sgn(x)$ where
\[ sgn(x) =\begin{cases}
		-1 & x<0\\
		0  & x=0 \\
		1 & x>1 \\
		\end{cases}\]
The Hilbert  transform can  be intuitively considered to be a filter which simply shifts phases of all frequency components of its input by $\frac{-\pi}{2}$ radians. Using the transform, we can construct  an "analytic" (complex time) signal Y(t)  as follows:
\[Y(t) = y(t) + j h(t)\]
 where y(t) corresponds to the original signal and h(t) corresponds to the Hilbert transform of the original signal.

To extract the envelope e(t) of a signal x(t) , we  consider only  the magnitude of the analytic signal as shown by the following equation.
...
Figure 8.3 provides an illustration of the envelope extraction process. The topmost plot corresponds to an instance of digit `5' from the TIDIGITS corpus. The second plot corresponds to the envelope of the raw signal.  If we represent each raw signal as an image than the envelope of the signal will correspond to its perimeter which is an outline of the upper and lower parts of the signal. Hence, to capture the envelope of both the upper part and lower part  of the signal, I  applied the Hilbert transform twice: once on the positive part of the signal to extract the top envelope and second on the negative part of the signal to extract the bottom envelope. The second plot  represents the augmentation of the  upper and lower envelopes of the speech utterance. However, human speech is spoken at a very high frequency. Thus as evident from the figure, the extracted envelopes  have very high frequencies. To remove these unnecessary details, I performed wavelet decomposition to extract a curve that captures the envelope of the utterance as shown by the third plot.


\begin{figure}[H]
  
      \includegraphics[scale=0.5]{83.jpg}
 \caption{The top plot corresponds to an utterance of digit 5. The second plot corresponds to the envelope of the signal extracted by the Hilbert transform. The third plot corresponds to a smoothed version of the envelope extracted by wavelet decomposition and the bottom plot corresponds to the curvatures associated with the smoothed envelope  }
  \end{figure}
 ...
 To investigate whether the proposed feature extraction methodology improves the performance of the 1NN classifier, I have the accuracy and the run time of the classifier against the baseline 1NN classifier employed on the rescaled speech utterances. A summary of the results are as follows:
