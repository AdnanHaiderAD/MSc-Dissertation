...
To study how  the performance of the DTW algorithm is affected by the proposed changes,  I tested this algorithm on 3 different types of data sets and compared 
it's performance against the following  4 domain dependent and independent methodologies we discussed so far :
\begin{itemize}
\item Methodology 1
\begin{itemize}
\item Apply feature selection process to remove segments of silece and down sample the remaining segment to improve the quality.
\item  Apply valuebased DTW(which we denote as baseline) using a window constraint and a euclidean metric.
\end{itemize}
...
\item Apply  the feature extraction process discussed in [] to extract local and global features.
 \item Apply DTW using a window constraint and a euclidean metric.
 \end{itemize}
...
\begin{itemize}
 ...
 \item Apply feature selection process that only removes  segments of  silence 
 \item Extract MFCC features
 \item Apply DTW augmented witha euclidean metric.
 \end{itemize}
 \end{itemize}
