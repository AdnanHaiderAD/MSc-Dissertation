...
%
%   STEP(SYS) plots the step response of the dynamic system SYS. For systems 
...
%
%   STEP(SYS,T) uses the time vector T for simulation (expressed in the time
%   units of SYS). For discrete-time models, T should be of the form Ti:Ts:Tf 
%   where Ts is the sampling time. For continuous-time models, T should be of 
%   the form Ti:dt:Tf where dt is the sampling period for the discrete 
%   approximation of SYS. The step input is always assumed to start at t=0 
%   (regardless of Ti).
%
...
%
%   [Y,T] = STEP(SYS) returns the output response Y and the time vector T used
%   for simulation. No plot is drawn on the screen. If SYS has NY outputs and
%   NU inputs, and LT = length(T), Y is an array of size [LT NY NU] where 
%   Y(:,:,j) gives the step response of the j-th input channel. The time vector 
%   T is expressed in the time units of SYS. 
%
...
%	$Revision: 1.1.8.4 $  $Date: 2010/11/08 02:16:13 $
