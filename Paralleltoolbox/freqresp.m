...
%
%   H = FREQRESP(SYS,W) computes the frequency response H of the dynamic system 
%   SYS at the frequencies specified by the vector W. These frequencies should 
%   be real and expressed in radians/TimeUnit (relative to the time units 
%   specified in SYS.TimeUnit, the default being seconds).
%
%   H = FREQRESP(SYS,W,UNITS) explicitly specifies the frequency units of W.
%   The string UNITS can be any of the following: 'rad/TimeUnit',
%   'cycles/TimeUnit', 'rad/s', 'Hz', 'kHz', 'MHz', 'GHz', or 'rpm'.
%
%   If SYS has NY outputs and NU inputs, and W contains NW frequencies, the
%   output H is a NY-by-NU-by-NW array such that H(:,:,k) gives the response
%   at the frequency W(k).
%
%   If SYS is a S1-by-...-Sp array of systems with NY outputs and NU inputs,
%   then SIZE(H)=[NY NU NW S1 ... Sp] where NW=LENGTH(W).
%
%   Note: FREQRESP is optimized for medium-to-large vectors of frequencies.
%   Use EVALFR for a handful of frequencies.
%
%   See also DYNAMICSYSTEM/EVALFR, BODE, SIGMA, NYQUIST, NICHOLS, DYNAMICSYSTEM.
...
%   $Revision: 1.1.8.4 $  $Date: 2010/10/25 15:54:26 $
